                        ...
                        <div id="slides" class="slides p-2 row">
                            @foreach($slides as $slide)
                                ...
                                <div class="slide px-2 my-2 col-12 col-md-4" data-id="{{ $slide->id }}">
                                    <div class="px-3 py-2 d-flex align-items-center" style="opacity: {{ ($slide->activo == '0') ? '.45' : '1' }}; border: dashed 2px #00ADBA;">
                                        @if($slide->imagen)
                                            <img src="/uploads/{{ $slide->imagen }}" loading="lazy" class="img-fluid" alt="">
                                        @endif
                                    </div>
                                </div>
                            @endforeach
...
@section('customCSS')
    ...
    <style>
        .slide h3{ font-weight: 600; margin:0; }
        .slide{
            cursor: move;
        }
        .slide:hover{
    ...
    <script>
        $(function(){
            ...
            $("#filtrar").on('click', function(){
                location.href="{{ route('slides.ordenar_form') }}?lang="+$("#idioma").val();
            });
