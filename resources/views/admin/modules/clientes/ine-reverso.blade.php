@if($cliente->ine_reverso)
    <img src="/uploads/{{ $cliente->ine_reverso }}" class="img-fluid" loading="lazy" alt="">
    <div></div>
    <a href="#." class="btn btn-danger btn-xs btn-eliminar-ine-reverso">
    	<i class="fa fa-trash"></i> Eliminar
    </a>
@else
    <a href="#." class="btn btn-success btn-xs" data-toggle="modal" data-target="#modalAgregarIneReverso">
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="formAgregarIneReverso" action="{{ route('clientes.agregar_ine_reverso') }}" data-focus="false" data-disable="false">
                        <input type="hidden" name="cliente_id" value="{{ $cliente->id }}">
                        <div class="form-group mb-2">
                            <label for="">INE Reverso</label>
                            <x-file-input name="ine_reverso" route="{{ route('uploads_subir') }}" folder="clientes/ine" filecount="1" scripts="0" />
                        </div>
                        <div class="form-group mb-2">
	...
	<script>
		$(function(){
			...
			$('body').on('click','.btn-eliminar-ine-reverso',function(){
                let url = "{{ route('clientes.eliminar_ine_reverso') }}";
